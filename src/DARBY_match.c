...
static bool
DARBY_verse_matches(const DARBY_ref *ref, const DARBY_verse *verse)
{
    switch (ref->type) {
        case KJV_REF_SEARCH:
...
static int
DARBY_chapter_bounds(int i, int direction, int maximum_steps)
{
    assert(direction == KJV_DIRECTION_BEFORE || direction == KJV_DIRECTION_AFTER);

    int steps = 0;
    for ( ; 0 <= i && i < DARBY_verses_length; i += direction) {
        bool step_limit = (maximum_steps != -1 && steps >= maximum_steps) ||
            (direction == KJV_DIRECTION_BEFORE && i == 0) ||
            (direction == KJV_DIRECTION_AFTER && i + 1 == DARBY_verses_length);
        if (step_limit) {
            break;
        }

        const DARBY_verse *current = &DARBY_verses[i], *next = &DARBY_verses[i + direction];
...
static int
DARBY_next_match(const DARBY_ref *ref, int i)
{
    for ( ; i < DARBY_verses_length; i++) {
        const DARBY_verse *verse = &DARBY_verses[i];
        if (DARBY_verse_matches(ref, verse)) {
            return i;
        }
    }
    return -1;
...
static void
DARBY_next_addrange(DARBY_next_data *next, DARBY_range range) {
...
{
    if (next->current >= DARBY_verses_length) {
        return -1;
    }

    if (next->matches[0].start != -1 && next->matches[0].end != -1 && next->current >= next->matches[0].end) {
        next->matches[0] = next->matches[1];
        next->matches[1] = (DARBY_range){-1, -1};
    }

    if ((next->next_match == -1 || next->next_match < next->current) && next->next_match < DARBY_verses_length) {
        int next_match = DARBY_next_match(ref, next->current);
        if (next_match >= 0) {
            next->next_match = next_match;
            DARBY_range bounds = {
                .start = DARBY_chapter_bounds(next_match, KJV_DIRECTION_BEFORE, config->context_chapter ? -1 : config->context_before),
                .end = DARBY_chapter_bounds(next_match, KJV_DIRECTION_AFTER, config->context_chapter ? -1 : config->context_after) + 1,
            };
            DARBY_next_addrange(next, bounds);
        } else {
            next_match = DARBY_verses_length;
        }
    }
