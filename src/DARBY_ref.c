...
DARBY_newref()
{
    return calloc(1, sizeof(DARBY_ref));
}
...
static bool
DARBY_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
DARBY_book_matches(const DARBY_book *book, const char *s)
{
    return DARBY_bookequal(book->name, s, false) ||
        DARBY_bookequal(book->abbr, s, false) ||
        DARBY_bookequal(book->name, s, true);
}

static int
DARBY_book_fromname(const char *s)
{
    for (int i = 0; i < DARBY_books_length; i++) {
        const DARBY_book *book = &DARBY_books[i];
        if (DARBY_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
DARBY_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
DARBY_parseref(DARBY_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (DARBY_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = DARBY_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
